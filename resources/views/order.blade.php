      ...
      <div class="bg-white order shadow">
        ...
        <div class="order-top mt-2 row">
            <div class="col-md-4 col-lg-4 col-xs-12 col-sm-12 float-start">
                <img src="/assets/img/{{ $menu->image }}" class="order-img" alt="{{ $menu->name }}">
            </div>
            <div class="col-md-7 col-lg-7 order-detail float-end">
                <div class="order-detail-top mb-3">
                    <div class="order-detail-title">
                        <h2 class="order-detail-title-text">{{ $menu->name }}</h2>
                    </div>
                    <div class="order-detail-description">
                        <p class="order-detail-description-text">{{ $menu->excerpt }}</p>
                    </div>
                </div>
                <div class="order-detail-bottom mb-0 ">
                    <div class="order-detail-price-strike">
                        <p class="order-detail-price-strike-text text-secondary"><?php $harga_lama = number_format($menu->old_price,0,"","."); echo 'Harga Lama : Rp. ' . $harga_lama; ?></span></p>
                    </div>
                    <div class="order-detail-price ">
                        <p class="order-detail-price-text">Harga : <span class="text-primary"><?php $harga = number_format($menu->price,0,"","."); echo 'Rp. '.$harga; ?></span></p>
                    </div>
                </div>
            </div>
        </div>
        <!-- Order middle -->
        <form name="addtoCart" method="post" action="{{ '/add-cart' }}">{{ csrf_field() }}
            ...
            <div class="col-md-4 col-lg-4 col-sm-12 col-xs-12 float-start ">
                ...
                <input type="hidden" id="price" value="{{ $menu->price }}" name="price">
            ...
            <div class="col-md-7 col-lg-7 col-sm-12 col-xs-12  float-end">
                ...
                <input type="number" step="1" min="0" max="100" value="1" name="quantity" class="form-control" id="jumlah">
